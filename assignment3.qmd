...
author: "Troy (Shengkun) Liu & Zupeng Zeng"
format: pdf
editor: visual
---
...
```{r}
## Pick the "East" neighbors as text, then make URL slugs like "Kenwood,_Chicago"
east_col <- which(grepl("east", names(GB_adjacent), ignore.case = TRUE))

if (length(east_col) == 1) {
  east_of_GB <- unique(na.omit(GB_adjacent[[east_col]]))
} else {
  # fallback: use your flattened vector and keep likely east neighbors
  east_of_GB <- unique(GB_adjacent_vec)
  east_of_GB <- east_of_GB[!grepl("Grand Boulevard|Chicago|^\\s*$", east_of_GB)]
}

east_pages <- paste0(gsub(" ", "_", east_of_GB), ",_Chicago")
east_pages
```
...
```{r}
# Grand Boulevard page text (single block)
...
pages <- c("Grand_Boulevard,_Chicago", "Kenwood,_Chicago", "Oakland,_Chicago", "Hyde_Park,_Chicago")
